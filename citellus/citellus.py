...
def parse_args(default=False, parse=False):
    ...
    s.add_argument("--no-read-config", default=False, help=_("Do not read configuration from file %s or ~/.citellus.conf" % os.path.join(citellusdir, "citellus.conf")), action="store_true")
...
def diff_config(options, defaults=parse_args(default=True), path=False):
    ...
    for key in vars(options):
        ...
        if keyset != keydef and key != 'dump_config' and key != 'no_read_config' and key != 'plugin_path':
...
def main():
    ...
    if options.dump_config:
        options.no_read_config = True
        print(dump_config(options))
        sys.exit(0)

    if not options.no_read_config:
