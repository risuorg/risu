...
def main():
    ...
    if options.read_config:
        ...
        if diff_config(options=savedconfig) != {}:
            ...
            newoptions = json.loads(dump_config(savedconfig))
            newoptions.update(json.loads(dump_config(options, path=True)))
            options = array_to_config(newoptions)
    ...
    LOG.debug("# CLI options: %s" % diff_config(options=options, path=True))
    LOG.debug("# Using saved options: %s" % diff_config(options=savedconfig))
