...
if [ "x$CITELLUS_LIVE" = "x0" ];  then
  if [ ! -f "${CITELLUS_ROOT}/sos_commands/systemd/systemctl_list-units_--all" ]; then
    ...
    if grep -q "ceph-mon.*active" "${CITELLUS_ROOT}/sos_commands/systemd/systemctl_list-units_--all"; then
      if [ ! -f "${CITELLUS_ROOT}/sos_commands/ceph/ceph_osd_dump" ]; then
        ...
        for pool in $(sed -n -r -e 's/^pool.*\x27(.*)\x27.*$/\1/p' \
                ${CITELLUS_ROOT}/sos_commands/ceph/ceph_osd_dump); do
          MIN_SIZE=$(sed -n -r -e "s/^pool.*$pool.*min_size[ \t]([0-9]).*$/\1/p" \
                ${CITELLUS_ROOT}/sos_commands/ceph/ceph_osd_dump )
          SIZE=$(sed -n -r -e "s/^pool.*$pool.*\ssize[ \t]([0-9]).*$/\1/p" \
                ${CITELLUS_ROOT}/sos_commands/ceph/ceph_osd_dump )
          _MIN_SIZE="$(( (SIZE/2) + 1 ))"
  ...
  if hiera -c /etc/puppet/hiera.yaml enabled_services | egrep -sq ceph_mon; then
    for pool in $(ceph osd dump | sed -n -r -e 's/^pool.*\x27(.*)\x27.*$/\1/p')
    do
      MIN_SIZE=$(ceph osd dump | sed -n -r -e "s/^pool.*$pool.*min_size[ \t]([0-9]).*$/\1/p")
      SIZE=$(ceph osd dump | sed -n -r -e "s/^pool.*$pool.*\ssize[ \t]([0-9]).*$/\1/p")
      _MIN_SIZE="$(( (SIZE/2) + 1 ))"
