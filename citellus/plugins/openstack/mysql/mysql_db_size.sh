...
then
    ...
    echo "$( du -mskc ${MYSQL_DIR}/* | sort -nr |  awk -F '\t' -v OFS='\t' '{if ($1 > 1024**2) $1 = sprintf("%.1fG",$1/1024**2); else if ($1 > 1024) $1 = sprintf("%.1fM",$1/1024); else $1 = sprintf("%sK",$1)} 1'| egrep -i "galera.cache|ibdata|ib_log")"
    exit $RC_OKAY
else
