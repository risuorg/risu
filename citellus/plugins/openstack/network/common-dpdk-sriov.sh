...
if is_lineinfile "Intel" "${CITELLUS_ROOT}/proc/cpuinfo"; then
    if ! is_lineinfile "intel_iommu=on" "${CITELLUS_ROOT}/proc/cmdline"
        echo $"missing intel_iommu=on on kernel cmdline" >&2
        flag=1
    fi
    if ! is_lineinfile "iommu=pt" "${CITELLUS_ROOT}/proc/cmdline"
        echo $"missing iommu=pt on kernel cmdline" >&2
        flag=1
    fi
else
    if ! is_lineinfile "amd_iommu=pt" "${CITELLUS_ROOT}/proc/cmdline"
