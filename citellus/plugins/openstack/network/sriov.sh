...
is_lineinfile "${CITELLUS_ROOT}/etc/tuned/cpu-partitioning-variables.conf" "^isolated_cores=.*" || echo "missing isolated_cores in /etc/tuned/cpu-partitioning-variables.comf" >&2  && flag=1
...
is_lineinfile "${CITELLUS_ROOT}/etc/nova/nova.conf" "^scheduler_defaults.*PciPassthroughtFilter" || echo "missing PciPassthroughFilter in nova.conf" > &1 && flag=1
is_lineinfile "${CITELLUS_ROOT}/etc/nova/nova.conf" "^vcpu_pin_set.*" || echo "missing vcpu_pin_set in nova.conf" > &1 && flag=1
