...
mapping_os_versions(){
  if [[ -z $rhos_release || -z $os_project_name ]] ; then
    ...
    releases=$(curl -s https://releases.openstack.org/ |awk '/^<tr.*class="doc">/ {
      t=gensub(/^<tr.*class="doc">(\w+).*$/, "\\1", "g", $0); print t }' | tac)
    counter="1"
    for release in $releases; do
      osproject_2_osv[$release]=$counter
      nova_version=$(curl -s https://releases.openstack.org/${release,,}/index.html |
        awk 'BEGIN {RS="<tr"} /class="std/ && />nova</ {
        print gensub(/<td>([0-9]+\.[0-9]+).*<\/td>/, "\\1", "g", $7)}')
      nova_versions[$nova_version]=$release
      counter=$[ $counter + 1 ]
    done
    os_release=$(is_rpm openstack-nova-common | awk 'BEGIN {FS="."} ; {print gensub(/openstack-nova-common-([0-9]+).*/, "\\1", $1)"."$2}')
