        ...
        if data[pluginid]['id'] == citellus.calcid(string='/plugins/faraday/positive/network/mtus.sh'):
            ...
            for pair in err.split(";"):
                if pair != '':
                    ...
                    newid = "%s-%s" % (id, citellus.calcid(string=pair))
                    update = {'id': newid, 'description': '%s: %s' % (desc, pair), 'long_name': '%s: %s' % (ln, pair), 'plugin': '%s-%s' % (plugpath, pair)}
