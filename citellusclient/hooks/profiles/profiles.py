...
def plugidsforprofile(profile, plugins):
    ...
    citellus.LOG.debug("IRANZO plugidsforprofile")
    citellus.LOG.debug(ids)
...
def run(data, quiet=False):  # do not edit this line
    ...
    for item in profiles:
        ...
        profile = item['plugin']
        citellus.LOG.debug("Processing profile %s:" % profile)
        ...
        for id in plugidsforprofile(profile=profile, plugins=plugins):
            citellus.LOG.debug("IRANZO")
            citellus.LOG.debug(data[id]['plugin'])
            citellus.LOG.debug(data[id])
            data[uid]['result']['err'] = data[uid]['result']['err'] + "\n" + "%s" % {'plugin': data[id]['plugin'], 'err': data[id]['result']['err'], 'rc': data[id]['result']['rc']}
