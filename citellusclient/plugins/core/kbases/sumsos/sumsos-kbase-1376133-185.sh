...
# long_name: Finds matches of kernel: gsch_get_file_event\(\) open\(.*\(.*\)\) failed: .*
# description: Finds matches of error kernel: gsch_get_file_event\(\) open\(.*\(.*\)\) failed: .*

REGEXP="kernel: gsch_get_file_event\(\) open\(.*\(.*\)\) failed: .*"
KCS=1376133
