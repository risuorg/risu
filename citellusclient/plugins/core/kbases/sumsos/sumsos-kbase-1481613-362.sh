#!/bin/bash
    ...
    # long_name: Finds matches of kernel: sd .*: megasas: RESET -.* cmd=.* retries=.*
    # description: Finds matches of error kernel: sd .*: megasas: RESET -.* cmd=.* retries=.*

    REGEXP="kernel: sd .*: megasas: RESET -.* cmd=.* retries=.*"
    KCS=1481613
