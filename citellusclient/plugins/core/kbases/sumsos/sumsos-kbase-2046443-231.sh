#!/bin/bash
    ...
    # long_name: Finds matches of kernel: Pid: .*\(.*\) over core_pipe_limit
    # description: Finds matches of error kernel: Pid: .*\(.*\) over core_pipe_limit

    REGEXP="kernel: Pid: .*\(.*\) over core_pipe_limit"
    KCS=2046443
