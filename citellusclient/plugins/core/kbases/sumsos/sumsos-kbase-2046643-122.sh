#!/bin/bash
    ...
    # long_name: Finds matches of kernel: lpfc .*: .*:0127 ELS timeout Data: x.* x.* x.* x.*
    # description: Finds matches of error kernel: lpfc .*: .*:0127 ELS timeout Data: x.* x.* x.* x.*

    REGEXP="kernel: lpfc .*: .*:0127 ELS timeout Data: x.* x.* x.* x.*"
    KCS=2046643
