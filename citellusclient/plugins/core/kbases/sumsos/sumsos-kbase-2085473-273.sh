...
# long_name: Finds matches of kernel: Write-error on swap-device \(.*:.*\)
# description: Finds matches of error kernel: Write-error on swap-device \(.*:.*\)

REGEXP="kernel: Write-error on swap-device \(.*:.*\)"
KCS=2085473
