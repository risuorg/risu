#!/bin/bash
    ...
    # long_name: Finds matches of kernel: BUG: soft lockup - CPU#.* stuck for .*s! .*
    # description: Finds matches of error kernel: BUG: soft lockup - CPU#.* stuck for .*s! .*

    REGEXP="kernel: BUG: soft lockup - CPU#.* stuck for .*s! .*"
    KCS=21849
