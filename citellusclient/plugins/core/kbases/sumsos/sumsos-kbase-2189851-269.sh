#!/bin/bash
    ...
    # long_name: Finds matches of kernel: .*: sending ioctl .* to a partition!
    # description: Finds matches of error kernel: .*: sending ioctl .* to a partition!

    REGEXP="kernel: .*: sending ioctl .* to a partition!"
    KCS=2189851
