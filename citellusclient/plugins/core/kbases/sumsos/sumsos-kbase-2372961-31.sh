#!/bin/bash
    ...
    # long_name: Finds matches of lvm.*: /.*: open failed: Too many open files
    # description: Finds matches of error lvm.*: /.*: open failed: Too many open files

    REGEXP="lvm.*: /.*: open failed: Too many open files"
    KCS=2372961
