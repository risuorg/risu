#!/bin/bash
    ...
    # long_name: Finds matches of kernel: st: Too many tape devices \(max. 128\)
    # description: Finds matches of error kernel: st: Too many tape devices \(max. 128\)

    REGEXP="kernel: st: Too many tape devices \(max. 128\)"
    KCS=24699
