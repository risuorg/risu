#!/bin/bash
    ...
    # long_name: Finds matches of kernel: IPv4: martian source .* from .*, on dev eth.*
    # description: Finds matches of error kernel: IPv4: martian source .* from .*, on dev eth.*

    REGEXP="kernel: IPv4: martian source .* from .*, on dev eth.*"
    KCS=25157
