#!/bin/bash
    ...
    # long_name: Finds matches of kernel: sd .*: .*Sense Key : Hardware Error .*
    # description: Finds matches of error kernel: sd .*: .*Sense Key : Hardware Error .*

    REGEXP="kernel: sd .*: .*Sense Key : Hardware Error .*"
    KCS=271923
