#!/bin/bash
    ...
    # long_name: Finds matches of kernel: pci .*: BAR .*: failed to assign \[io  size 0x.*\]
    # description: Finds matches of error kernel: pci .*: BAR .*: failed to assign \[io  size 0x.*\]

    REGEXP="kernel: pci .*: BAR .*: failed to assign \[io  size 0x.*\]"
    KCS=2772311
