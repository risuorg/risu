...
# long_name: Finds matches of qdiskd.*: qdiskd on node .* reports hung read\(\)
# description: Finds matches of error qdiskd.*: qdiskd on node .* reports hung read\(\)

REGEXP="qdiskd.*: qdiskd on node .* reports hung read\(\)"
KCS=27764
