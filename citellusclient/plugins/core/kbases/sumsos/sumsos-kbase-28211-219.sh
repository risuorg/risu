#!/bin/bash
    ...
    # long_name: Finds matches of kernel: nfs: server .* not responding, still trying
    # description: Finds matches of error kernel: nfs: server .* not responding, still trying

    REGEXP="kernel: nfs: server .* not responding, still trying"
    KCS=28211
