#!/bin/bash
    ...
    # long_name: Finds matches of kernel: nfs: server .* not responding, timed out
    # description: Finds matches of error kernel: nfs: server .* not responding, timed out

    REGEXP="kernel: nfs: server .* not responding, timed out"
    KCS=28211
