#!/bin/bash
    ...
    # long_name: Finds matches of kernel: nfsacl: server .* not responding, timed out
    # description: Finds matches of error kernel: nfsacl: server .* not responding, timed out

    REGEXP="kernel: nfsacl: server .* not responding, timed out"
    KCS=28211
