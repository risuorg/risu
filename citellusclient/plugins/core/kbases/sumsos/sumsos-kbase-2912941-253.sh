#!/bin/bash
    ...
    # long_name: Finds matches of kernel: sfc .* .*: MC command 0x.* inlen .* failed rc=.* \(raw=.*\) arg=.*
    # description: Finds matches of error kernel: sfc .* .*: MC command 0x.* inlen .* failed rc=.* \(raw=.*\) arg=.*
