...
# long_name: Finds matches of lvm: Found duplicate PV .*: using /dev/.* not /dev/.*
# description: Finds matches of error lvm: Found duplicate PV .*: using /dev/.* not /dev/.*

REGEXP="lvm: Found duplicate PV .*: using /dev/.* not /dev/.*"
KCS=2989
