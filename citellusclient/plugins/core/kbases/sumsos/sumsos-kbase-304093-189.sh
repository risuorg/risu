#!/bin/bash
    ...
    # long_name: Finds matches of kernel: hpsa .*: Abort request on .*
    # description: Finds matches of error kernel: hpsa .*: Abort request on .*

    REGEXP="kernel: hpsa .*: Abort request on .*"
    KCS=304093
