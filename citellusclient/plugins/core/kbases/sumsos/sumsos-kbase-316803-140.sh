#!/bin/bash
    ...
    # long_name: Finds matches of kernel: lpfc .*: .*:1477 Failed to set up hba
    # description: Finds matches of error kernel: lpfc .*: .*:1477 Failed to set up hba

    REGEXP="kernel: lpfc .*: .*:1477 Failed to set up hba"
    KCS=316803
