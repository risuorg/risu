#!/bin/bash
    ...
    # long_name: Finds matches of kernel: vxfs: Unknown symbol .* \(err -22\)
    # description: Finds matches of error kernel: vxfs: Unknown symbol .* \(err -22\)

    REGEXP="kernel: vxfs: Unknown symbol .* \(err -22\)"
    KCS=3400841
