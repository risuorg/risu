#!/bin/bash
    ...
    # long_name: Finds matches of kernel: sd .*: .*Sense Key : Recovered Error .*
    # description: Finds matches of error kernel: sd .*: .*Sense Key : Recovered Error .*

    REGEXP="kernel: sd .*: .*Sense Key : Recovered Error .*"
    KCS=41612
