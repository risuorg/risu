#!/bin/bash
    ...
    # long_name: Finds matches of kernel: connection.*:0: detected conn error \(.*\)
    # description: Finds matches of error kernel: connection.*:0: detected conn error \(.*\)

    REGEXP="kernel: connection.*:0: detected conn error \(.*\)"
    KCS=423473
