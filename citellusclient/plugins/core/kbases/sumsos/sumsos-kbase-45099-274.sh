#!/bin/bash
    ...
    # long_name: Finds matches of kernel: qla2xxx .*: scsi\(.*\): Abort command issued -- .*
    # description: Finds matches of error kernel: qla2xxx .*: scsi\(.*\): Abort command issued -- .*

    REGEXP="kernel: qla2xxx .*: scsi\(.*\): Abort command issued -- .*"
    KCS=45099
