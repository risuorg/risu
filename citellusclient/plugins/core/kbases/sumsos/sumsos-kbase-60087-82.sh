#!/bin/bash
    ...
    # long_name: Finds matches of udevd.*: worker .* failed while handling .*
    # description: Finds matches of error udevd.*: worker .* failed while handling .*

    REGEXP="udevd.*: worker .* failed while handling .*"
    KCS=60087
