#!/bin/bash
    ...
    # long_name: Finds matches of multipathd: .*: stop event checker thread
    # description: Finds matches of error multipathd: .*: stop event checker thread

    REGEXP="multipathd: .*: stop event checker thread"
    KCS=62041
