#!/bin/bash
    ...
    # long_name: Finds matches of kernel: scsi: host .* channel .* id .* lun.* has a LUN larger than allowed by the host adapter
    # description: Finds matches of error kernel: scsi: host .* channel .* id .* lun.* has a LUN larger than allowed by the host adapter
