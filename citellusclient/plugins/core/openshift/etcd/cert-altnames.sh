...
if is_enabled etcd || is_enabled etcd_container ; then
    if openssl x509 -noout -text -in "${CITELLUS_ROOT}"/etc/etcd/server.crt | grep 'X509v3 Subject Alternative Name' -A1 | grep -q DNS:$(hostname -f) ; then 
	...
	openssl x509 -noout -text -in "${CITELLUS_ROOT}"/etc/etcd/server.crt | grep 'X509v3 Subject Alternative Name' -A1  >&2
        exit ${RC_FAILED}
    fi
else
