...
if is_lineinfile "haproxy.*SSL handshake failure" "${CITELLUS_ROOT}/var/log/messages"; then
    grep "haproxy.*SSL handshake failure" "${CITELLUS_ROOT}/var/log/messages"| tail >&2
    exit ${RC_FAILED}
fi
exit ${RC_OKAY}
