...
# Copyright (C) 2017   Jaison Raju (jraju@redhat.com)
...
# We are depending on any instance's libvirt xml to be present in sosreport to identify if the computes have instances which use cpu_policy dedicated.
	...
	for libvirt_xml in `ls ${CITELLUS_ROOT}/etc/libvirt/qemu/instance-*.xml`
	do
	        if $(grep -q vcpupin $libvirt_xml)
        	then
                	pinned=$(xmllint --xpath "string(//name)" $libvirt_xml)
	                pinned=""+$pinned
        	elif $(grep -q vcpupin $libvirt_xml)
	        then
        	        unpinned=$(xmllint --xpath "string(//name)" $libvirt_xml)
                	unpinned=""+$unpinned
	        fi
	...
	then
		...
		if ! is_lineinfile "^isolated_cores=.*" "${CITELLUS_ROOT}/etc/tuned/cpu-partitioning-variables.conf";then
			...
			TUNED_CORES=$(grep "^isolated_cores=.*" "${CITELLUS_ROOT}/etc/tuned/cpu-partitioning-variables.conf" |cut -f2 -d\= |tr ",\'\"" "\n")
        	        TUNED_CORES=$(expand_and_remove_excludes ${VCPUPINSET})

			#check if vcpu_pin_set cores are tuned & tuned profile is set
	                misconfigured_cores=0
        	        for i in $VCPUPINSET
                	do
                        	if [ ! $(grep -q " $i " $TUNED_CORES) ]
	                        then
        	                        misconfigured_cores=1
                	        fi
	                done
	...
	# check if vcpu cores are isolated
        	...
        	then
	                ...
	                if [ $misconfigured_cores = "1" ]
        	        then
                	        echo -ne "Compute is hosting instances with cpu_policy, hence it should isolated cores used by nova (vcpu_pin_set) in kernel using isolcpus. \nvcpu_pin_set cores $(VCPUPINSET) are not isolated in kernel via isolcpus $ISOLCPUS.\nCPU cores used by instances with dedicated cpu_policy should be isolated using isolcpus in kernel for better performance https://access.redhat.com/solutions/2884991" >&2
	    			flag=1
	                fi
        	fi
		...
		numa_nodes=$(cat ${CITELLUS_ROOT}/sos_commands/processor/lscpu | grep "NUMA node(s)"| awk -F':[[:space:]]*' '{print $2}')
        	...
        	if [ $( cat ${CITELLUS_ROOT}/sos_commands/processor/lscpu | grep ^Thread | awk -F':[[:space:]]*' '{print $2}') = "2" ];
	        then
			# First get all the siblings.
                	...
                	for i in `seq 0 $(expr $numa_nodes - 1)` ;
	                do
		...
		do
			...
			for i in $instance_cpus
			do
				for j in `seq 0 $(expr $numa_nodes - 1)` ;
	                	do
					for k in `seq 0 $(expr $cores_per_socket -1)`
					do
						if [ $i = $sibling${j}0[$k] ];
						then 
							if [ ! $(echo $instance_cpus |egrep  -o " $sibling${j}1[$k] |^$sibling${j}1[$k] | $sibling${j}1[$k]$" | wc -l ) = '1' ]; 
							then
						...
						if [ $i = $sibling${j}1[$k] ];
						then 
							if [ ! $(echo $instance_cpus |egrep  -o " $sibling${j}0[$k] |^$sibling${j}0[$k] | $sibling${j}0[$k]$" | wc -l ) = '1' ]; 
							then
	                	...
	                	do
        	        		for k in `seq 0 $(expr $cores_per_socket -1)`
	                	        do
        	                		if [ $i = $sibling${j}0[$k] ];
                	                	then
	                	               		if [ ! $(echo $VCPUPINSET |egrep  -o " $sibling${j}1[$k] |^$sibling${j}1[$k] | $sibling${j}1[$k]$" | wc -l ) = '1' ];
	        	        	                then
	        	                        ...
	        	                        if [ $i = $sibling${j}1[$k] ];
        	        	                then
                	        	        	if [ ! $(echo $VCPUPINSET |egrep  -o " $sibling${j}0[$k] |^$sibling${j}0[$k] | $sibling${j}0[$k]$" | wc -l ) = '1' ];
                        	        	        then
        		...
        		do
	                	...
	                	do
	                	        ...
	                	        do
        	                	        if [ $i = $sibling${j}0[$k] ];
                	                	then
                        	                	if [ ! $(echo $TUNED_CORES |egrep  -o " $sibling${j}1[$k] |^$sibling${j}1[$k] | $sibling${j}1[$k]$" | wc -l ) = '1' ];
	                        	                then
	        	                        ...
	        	                        if [ $i = $sibling${j}1[$k] ];
        	        	                then
                	        	                if [ ! $(echo $TUNED_CORES |egrep  -o " $sibling${j}0[$k] |^$sibling${j}0[$k] | $sibling${j}0[$k]$" | wc -l ) = '1' ];
                        	        	        then
        		...
        		do
	                	...
	                	do
	                	        ...
	                	        do
        	                	        if [ $i = $sibling${j}0[$k] ];
                	                	then
                        	                	if [ ! $(echo $ISOLCPUS |egrep  -o " $sibling${j}1[$k] |^$sibling${j}1[$k] | $sibling${j}1[$k]$" | wc -l ) = '1' ];
	                        	                then
		...
		all_pinned_cores=$(echo 'cat //cputune/vcpupin/@cpuset' | xmllint --shell ${CITELLUS_ROOT}/etc/libvirt/qemu/instance-*.xml | awk -F\" 'NR % 2 == 0 { print $2 }')
		for i in $all_pinned_cores
		do
			if [ ! $(echo $all_pinned_cores |egrep  -o " $i |^$i | $i$" | wc -l ) = '1' ]
			then
				echo "Core $i is overlapping $(echo $all_pinned_cores |egrep  -o " $i |^$i | $i$" | wc -l ) time .\nThis may be an outcome of resize / live-migration on older version / live-migration using destination host which will by-pass nova scheduler.\nLive-Migration of pinned instances are not fully supported: https://access.redhat.com/solutions/2191071" >&2
				flag=1
			fi
		done	
	fi
