...
if [[ ${CITELLUS_LIVE} -eq 0 ]]; then
    ...
    NICS="$(ls ${CITELLUS_ROOT}/sos_commands/networking/)"
elif [[ ${CITELLUS_LIVE} -eq 1 ]];then
    FILE=$(mktemp)
    trap "rm ${FILE}" EXIT
    ovsdb-client -f list dump > ${FILE}
    NICS=$(ls /proc/sys/net/ipv*/conf |grep -v "sys/net"|sort|uniq)
fi

is_required_file "${FILE}"

PREPORTS="$(cat ${FILE}|egrep '(ha-|qr-|gq-)')"
PREPORTS="${PREPORTS} $(cat ${FILE}|grep name|grep tap)"

PORTS=`echo ${PREPORTS}|cut -d\" -f2`

echo "ports not in any router or namespace" >&2
flag=0

for port in ${PORTS}; do
         echo ${NICS} | grep $port &> /dev/null
        if [[ "$?" != "0" ]]; then
            echo $port >&2
            flag=1
        fi
