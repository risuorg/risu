...
# long_name: Finds matches of kernel: Error:Mpx:Path Bus .* Tgt .* Lun .* to .* is dead
# description: Finds matches of error kernel: Error:Mpx:Path Bus .* Tgt .* Lun .* to .* is dead

REGEXP="kernel: Error:Mpx:Path Bus .* Tgt .* Lun .* to .* is dead"
KCS=120943
