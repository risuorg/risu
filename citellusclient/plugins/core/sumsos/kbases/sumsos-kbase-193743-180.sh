...
# long_name: Finds matches of kernel: ch .*: attempting task abort! scmd\(.*\)
# description: Finds matches of error kernel: ch .*: attempting task abort! scmd\(.*\)

REGEXP="kernel: ch .*: attempting task abort! scmd\(.*\)"
KCS=193743
