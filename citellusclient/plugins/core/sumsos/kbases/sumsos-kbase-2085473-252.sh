...
# long_name: Finds matches of kernel: Read-error on swap-device \(.*:.*\)
# description: Finds matches of error kernel: Read-error on swap-device \(.*:.*\)

REGEXP="kernel: Read-error on swap-device \(.*:.*\)"
KCS=2085473
