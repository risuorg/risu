...
if [[ "x$CITELLUS_LIVE" = "x0" ]];  then
    ...
    kvm=$(grep "Virtualization=" "${CITELLUS_ROOT}/sos_commands/systemd/systemctl_show_--all" |cut -d "=" -f2)
    niccount=$(grep -c ^eth[0-9] "${CITELLUS_ROOT}/sos_commands/networking/ip_-s_-d_link")

elif [[ "x$CITELLUS_LIVE" = "x1" ]]; then
    dmesgfile=$(mktemp)
    dmesg > $dmesgfile
    kvm=$(systemd-detect-virt)
...
if grep -q "$ERRORMATCH" $dmesgfile; then
    if [[ "x$kvm" -ne "xkvm" ]]; then
        if ! [[ "x$niccount" -eq "x0" -o "x$niccount" -eq "x1" ]]; then
            echo $ERRORMSG >&2
            exit $RC_FAILED
        fi 
    fi
