...
class Event(object):
    def __init__(self, desc, time, context=None, status=None, index=None, duration_down=0, duration_bootloader=0,
                 duration_os=0):
        """
...
def main():
    ...
    for line in f:
        ...
        elif re.match(".* Runtime journal is using", line):
            ts = gettime(line)
    ...
    for i, e in enumerate(events):
        ...
        # calculates the duration of the downtime
            ...
            if mpe is not None:
                e.duration_downtime = (e.time - mpe.time).total_seconds()
    ...
    for i, e in enumerate(events):
        ...
        if e.duration_down > 600:
            ...
            numErrors += 1
        rebootList += format_rebootlist_ev.format(e.time, e.context, e.desc, e.status, e.duration_bootloader,
                                                  e.duration_os, e.duration_down)
    out = str(numErrors) + " problem(s) found:\n" + errorMsg + "\nEvents:\n" + format_rebootlist_hd.format('Time',
                                                                                                           'Context',
                                                                                                           'Description',
                                                                                                           'Status',
                                                                                                           'Boot', 'OS',
