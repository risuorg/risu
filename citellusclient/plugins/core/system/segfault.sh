...
# Copyright (C) 2017   Masaki Furuta (masaki@redhat.com)
...
if [[ "x$CITELLUS_LIVE" = "x0" ]];  then
    ...
    is_lineinfile "segfault at" "${journalctl_file}" && echo "SEGV detected" >&2 ; grep "segfault at" "${journalctl_file}" >&2 && exit ${RC_FAILED}

elif [[ "x$CITELLUS_LIVE" = "x1" ]]; then
    if journalctl -u kernel --no-pager --boot | grep -q "segfault at"; then
        echo "SEGV detected" >&2
        journalctl -u kernel --no-pager --boot | grep -q "segfault at" >&2
        exit ${RC_FAILED}
    fi
fi
