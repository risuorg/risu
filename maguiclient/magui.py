...
def autogroups(autodata):
    ...
    for item in autodata:
        for elem in item['sosreport'].iterkeys():
    ...
    for element in hostsdict:
        for item in hostsdict[element].iteritems():
    ...
    for category in groups:
        for subcategory in groups[category]:
            ...
            if len(groups[category][subcategory]) > 1 and len(groups[category][subcategory]) < len(hostsdict):
...
def main():
    ...
    if options.output:
        ...
        for group in autogroups(autodata):
            basefilename = os.path.splitext(options.output)
            filename = basefilename[0] + "-" + group + basefilename[1]
            print(filename)
            runmaguiandplugs(sosreports=sosreports, citellusplugins=citellusplugins, filename=filename)
