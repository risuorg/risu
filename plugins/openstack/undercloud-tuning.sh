...
then
  ...
  grep_file "/etc/heat/heat.conf" "^[ \t]*max_resources_per_stack.*=.*-1"
  grep_file "/etc/heat/heat.conf" "^[ \t]*num_engine_workers.*=.*4"

  # Checking /etc/my.cnf.d/server.cnf

  # Number of simultaneous connections to the database. The recommended
  # value is 4096. 
  grep_file "/etc/my.cnf.d/server.cnf" "^[ \t]*max_connections.*=.*4096"
  ...
  grep_file "/etc/my.cnf.d/server.cnf" "^[ \t]*innodb_additional_mem_pool_size.*=.*20M"
  ...
  grep_file "/etc/my.cnf.d/server.cnf" "^[ \t]*innodb_buffer_pool_size.*=.*1000M"
  ...
  grep_file "/etc/my.cnf.d/server.cnf" "^[ \t]*innodb_thread_concurrency.*=.*2"
  ...
  grep_file "/etc/nova/nova.conf" "^[ \t]*max_concurrent_builds.*=.*5"
else
  continue
fi
...
undercloud_tuning_check_${CHECK_MODE}
