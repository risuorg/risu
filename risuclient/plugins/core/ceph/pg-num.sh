...
if [[ "x$RISU_LIVE" == "x0" ]]; then
    if [[ -z ${systemctl_list_units_file} ]]; then
        ...
        if grep -q "ceph-mon.* active" "${systemctl_list_units_file}"; then
            ...
            is_required_file "${RISU_ROOT}/sos_commands/ceph/ceph_status"
            check_settings "${RISU_ROOT}/sos_commands/ceph/ceph_osd_dump" "${RISU_ROOT}/sos_commands/ceph/ceph_status"
        else
