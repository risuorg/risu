...
# long_name: DBErrors in OpenStack services
# description: Report any DBErrors in OpenStack services
# priority: 800
...
for log_file in ${log_files}; do
    [ -f "$log_file" ] || continue
    events=$(grep -i 'DBError' ${log_file} | grep -E -o '^([a-Z]+\ [0-9]+)|^([0-9\-]+)' | uniq -c | tail)
    if [[ -n ${events} ]]; then
