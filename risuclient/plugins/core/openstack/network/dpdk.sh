...
# long_name: DPDK Network Configuration
# description: Checks for various DPDK network configuration parameters
# priority: 870
...
if [[ -f $FILE ]]; then
    ...
    procids=$(grep ^processor ${RISU_ROOT}/proc/cpuinfo | cut -d ":" -f 2 | xargs echo)

    if is_lineinfile "^CPUAffinity.*" "${RISU_ROOT}/etc/systemd/system.conf"; then
        ...
        systemdaffinity=$(grep ^CPUAffinity ${RISU_ROOT}/etc/systemd/system.conf | cut -d "=" -f 2)
    ...
    elif is_lineinfile isolcpus ${RISU_ROOT}/proc/cmdline; then
