...
function taintcheck() {
    ...
    if [[ "x$TAINT_OOT_MODULE" == "x1" ]]; then
        ...
        if [[ "x$RISU_LIVE" == "x0" ]]; then
            if [[ -f "${RISU_ROOT}/sos_commands/kernel/dmesg" ]]; then
                grep "loading out-of-tree module taints kernel" "${RISU_ROOT}/sos_commands/kernel/dmesg" >&2
                echo >&2
            fi
        elif [[ "x$RISU_LIVE" == "x1" ]]; then
            dmesg | grep "loading out-of-tree module taints kernel" >&2
            echo >&2
        fi
    fi
    ...
    if [[ "x$TAINT_TECH_PREVIEW" == "x1" ]]; then
        ...
        if [[ "x$RISU_LIVE" == "x0" ]]; then
            if [[ -f "${RISU_ROOT}/sos_commands/kernel/dmesg" ]]; then
                grep "TECH PREVIEW" "${RISU_ROOT}/sos_commands/kernel/dmesg" >&2
                echo >&2
            fi
        elif [[ "x$RISU_LIVE" == "x1" ]]; then
            dmesg | grep "TECH PREVIEW" >&2
            echo >&2
        fi
    fi
}
