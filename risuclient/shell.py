...
def show_logo():
    ...
    logo = (
        r"__________.___  _____________ ___ ",
        r"\______   \   |/   _____/    |   \ ",
        r" |       _/   |\_____  \|    |   /",
        r" |    |   \   |/        \    |  / ",
        r" |____|_  /___/_______  /______/  ",
        r"        \/            \/          ",
        _("                                                  "),
    )
    print("\n".join(logo))
...
def findallplugins(options=None, filter=False):
    """
    Finds all plugins that risu recognized
...
def findplugins(
    ...
    if not folders:
        folders = [os.path.join(risudir, "plugins")]
    ...
    for plugin in plugins:
        subcategory = os.path.split(plugin)[0].replace(
            os.path.join(risudir, "plugins", extension), ""
        )
...
def dorisu(
    ...
    """
    Runs risu scripts on specified root folder
    :param pgstart: progress start
    :param pgend: progress end
    :param serveruri: server URL for risu.json upload via HTTP POST
    ...
    os.environ["RISU_ROOT"] = "%s" % path
    os.environ["RISU_LIVE"] = "%s" % RISU_LIVE
    os.environ["RISU_BASE"] = "%s" % risudir
    os.environ["RISU_TMP"] = "%s" % tempfile.mkdtemp()
    ...
    if savepath:
        ...
        filename = os.path.join(path, "risu.json")
...
def parse_args(default=False, parse=False):
    ...
    # Option parsing
    p = argparse.ArgumentParser("risu.py [arguments]", description=description)
    p.add_argument(
        "-l",
        "--live",
    ...
    p.add_argument(
        ...
        action="store_true",
        help=_(
            "Write results to JSON file risu.json and copy html interface in path defined in --output"
        ),
    )
    p.add_argument(
        "--run",
    ...
    s.add_argument(
        ...
        default=False,
        help=_(
            "Do not read configuration from file %s or ~/.risu.conf or .risu.conf"
            % os.path.join(risudir, "risu.conf")
        ),
        action="store_true",
    )
    ...
    s.add_argument(
        ...
        default="./",
        help=_(
            "Define path for .risu.conf configuration if not the default of ~/ or ./"
        ),
    )
    s.add_argument(
        "--call-home",
...
def read_config(options=False):
    ...
    #   - Overwritten with risu folder options
    #   - Overwritten with risu user options
    #   - Overwritten with risu CLI options
    ...
    possiblepaths = [
        os.path.join(risudir, "risu.conf"),
        os.path.join(risudir, "citellus.conf"),
    ]

    # Read custom config_path from arguments
    if options and options.config_path:
        custompath = os.path.join(options.config_path, ".risu.conf")
    else:
...
def anonymize(data, keeppath=False):
    ...
    if "magui" not in data["metadata"]["source"]:
        ...
        for ourdata in data["results"]:
            if "risu-outputs" in ourdata["name"]:
...
def write_results(
    ...
    if os.access(os.path.join(os.path.dirname(filename), "risu.html"), os.W_OK):
    ...
    if web:
        ...
        if basefolder == "":
            basefolder = "./"
        src = os.path.join(risudir, "risu.html")
...
def printresults(results, options):
    """
    Formats and prints results from risu execution
...
def main():
    ...
    if not options.live:
        if options.sosreport:
            ...
            RISU_ROOT = os.path.abspath(options.sosreport)
        elif (
        ...
        if options.list_categories:
            for plugin in plugins:
                ...
                category = os.path.split(plugin["plugin"])[0].replace(
                    os.path.join(risudir, "plugins", plugin["backend"]), ""
                )
    ...
    if not options.quiet:
        if options.live:
            ...
            print(_("mode: fs snapshot: "), "%s" % RISU_ROOT)
