...
if [ "x$CITELLUS_LIVE" = "x1" ];  then
  ...
  then
    ...
    if iptables -t nat -vnL | grep -q "REDIRECT.*169.254.169.254" ; then
      # Exit 0 means the framework is going to display 'okay' and move to another
      # script.
      exit 0
    else
      # Whenever we end with exit 1, we are printing out the content from the
  ...
  then
    # To make sure the file exists and we can check it, the exit 2 means we will skip the check
    ...
    # Here we check the rule in the file, if it exists we exit 0 and script reports okay.
